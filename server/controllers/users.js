...
const userByID = async (req, res, next, id) => {
  try {
    let user = await Users.findById(id).populate('following', '_id name')
...
const list = async (req, res) => {
  try {
    let users = await Users.find().select('name email updated created')
    res.json(users)
  } catch (err) {
    return res.status(400).json({
      error: errorHandler.getErrorMessage(err)
...
const addFollower = async (req, res) => {
  try{
    let result = await Users.findByIdAndUpdate(req.body.followId, {$push: {followers: req.body.userId}}, {new: true})
